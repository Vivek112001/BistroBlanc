   ...
   <div class="container"> 
     <table class="table table-striped table-hover table-bordered  m-0">
       ...
       <c:forEach items="${object4}" var="food">
     	<tr>
    		...
    		<c:set var="total" value="${total+food.item_id.price}"/>
    		<c:set var="tn" value="${tn},${food.item_id.item_name}" />
      		<c:set var="c1" value="${c1},${food.cartId}" />
    		<td colspan="2" style="font-size:20px;">
    		<div class="container bg-white border border-dark text-center" style="width:100px;">
    			<a href="deleteCartItem?cartId=${food.cartId}"><i class="fa-solid fa-circle-minus" style="color:#FF0000"></i></a>
    ...
    <div class="container">
			<div class="p-4 mb-3" style="background-color:#c6fcd49d;">
  			    ...
  			    <h3 class="text-primary"><span style="color:#451d5e">${user.name}</span>&nbsp|&nbsp<span style="color:#FF0000;">${user.mob_no}</span></h3>
            </div>
         ...
         <a href="completeOrder?email=<%=id%>&tn=${tn}&total=${total}&c1=${c1}" class="link-underline-light link-dark link-underline-opacity-0 link-offset-3-hover link-offset-2 ">
